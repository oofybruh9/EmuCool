#! /bin/bash
# This script installs the EmuCool package from a debian file online.
if [ ! -d $HOME/EmuCool ]; then
    mkdir -p $HOME/EmuCool
    # add nessesary folders and files
    mkdir themes
    mkdir config
    touch config/config.json
    touch config/systems.json
fi
cd $HOME/EmuCool

mkdir -p $HOME/ROMS

read -p "Do you want to auto create folders for ALL SYSTEMS (y/n): " answer
if [[ $answer != "y" && $answer != "Y" ]]; then
    echo "Ok, you can create folders manually."
    echo "more info in $HOME/ROMS/README.txt"
else
    # Create folders for all systems
    echo "Creating folders for all systems..."
    # Read the systems.json file and create folders
    if [ -f config/systems.json ]; then
        systems=$(jq -r '.[].name' config/systems.json)
        for system in $systems; do
            mkdir -p "$HOME/ROMS/$system"
        done
        echo "Folders created successfully."
    else
        echo "Error: systems.json file not found."
        exit 1
    fi
fi
touch README.txt
cat >$HOME/ROMS/README.txt <<EOL
Create a folder for each system in this directory and place your ROMs inside.
The folder names should match the system names in the systems.json file.
...
wget -q https://github.com/oofybruh9/EmuCool/releases/download/latest/EmuCool.deb
if [ $? -ne 0 ]; then
    echo "Error: Failed to download EmuCool debian file."
    exit 1
fi

date

sudo dpkg -i EmuCool.deb
if [ $? -ne 0 ]; then
    echo "Error: Failed to install EmuCool."
    exit 1
fi
# Clean up
rm EmuCool.deb
